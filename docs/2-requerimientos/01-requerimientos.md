...
| RF-62 | Año de cursada de la materia | Cada materia debe declarar en qué año de su carrera se cursa, y cada carrera cuántos años dura. El año de la materia no puede exceder la duración de su carrera, y no debe poder acortarse una carrera por debajo del año de sus materias vigentes. El año pertenece al plan de estudios y por eso vive en la materia y no en la comisión: en la comisión habría que repetirlo en cada una y dos comisiones de la misma materia podrían declarar años distintos, que es un estado imposible. |
| RF-63 | Docente propuesto al crear una comisión | Al elegir la materia, el formulario de comisión debe proponer su docente titular como docente asignado, dejándolo editable. Es quien dicta en la mayoría de los casos, y la excepción —otra persona a cargo de esa comisión— es justamente lo que hay que poder registrar. La propuesta no debe pisar un docente ya guardado al abrir el formulario en modo edición. |
| RF-61 | Exportación del reporte en PDF | El reporte de asistencias debe poder descargarse en PDF además de CSV. Son dos usos distintos: el CSV se sigue trabajando en una planilla y el PDF se imprime o se adjunta tal cual, de modo que el PDF lleva menos columnas, ninguna interna, y repite el encabezado en cada página. Debe dejar constancia del período, la cantidad de registros y la fecha de emisión: dos reportes impresos con filtros distintos no pueden ser indistinguibles. |
| RF-60 | Panel de inicio con estado del día | La pantalla de inicio debe informar el estado operativo del día —clases en curso con su docente y si ya marcó, conteo por estado y proporción del personal del día que ya registró asistencia— y listar las cargas incompletas que impiden el funcionamiento del reconocimiento: docentes sin consentimiento vigente, docentes sin modelo facial, y comisiones sin docente o sin horarios. **No debe repetir los accesos de la barra de navegación**: duplicarlos no agrega información y obliga a mantener dos listas de lo mismo. La ventana de "clase en curso" debe ser la misma que usa el pase, para que la pantalla no anuncie como marcable algo que el pase después rechaza. Las clases sin marca que aún no terminaron se cuentan aparte de las ausencias. |
...
| RNF-38 | Encabezado visible al recorrer un listado | Los listados con más filas de las que entran en pantalla deben desplazarse manteniendo el encabezado de la tabla a la vista. Sin eso hay que volver arriba para recordar qué columna se está mirando. |
| RNF-39 | Identificadores internos fuera de la interfaz | Ninguna pantalla debe mostrar el identificador de base de datos de un registro. No le sirve a quien opera y expone un detalle de implementación. |
...
| RNF-47 | Foco visible al navegar con teclado | Todo elemento enfocable debe mostrar un indicador visible cuando se llega a él con el teclado (WCAG 2.4.7). Cambiar solo el color del borde no alcanza: entre gris y azul hay poco contraste **entre estados** y con daltonismo son casi el mismo. El indicador no debe aparecer al hacer click con el mouse, donde es ruido. |
| RNF-48 | Salto al contenido | Debe existir un enlace que salte la navegación y lleve al contenido. Sin él, llegar al primer campo desde el teclado obliga a recorrer el menú completo en cada pantalla. |
| RNF-49 | Opciones no disponibles fuera del árbol de accesibilidad | Una opción que no se puede elegir debe salir del DOM, no solo ocultarse. `hidden` y `disabled` la sacan de la vista pero un lector de pantalla la sigue anunciando. |
| RNF-50 | Canal de códigos intercambiable | Por dónde sale el código de un solo uso debe ser configuración, no código. El sistema debe ofrecer al menos un canal de correo y uno de consola, seleccionables por propiedad, sin que los flujos que los usan se enteren. Sin esto, el alta de institución, la verificación de cuenta y la recuperación de contraseña quedan inutilizables cada vez que no hay un SMTP levantado, y no fallan a la vista: la pantalla dice "revisá tu correo" y no llega nada. El canal de consola debe avisar en el arranque de forma imposible de pasar por alto, porque dejarlo activo en producción abre cualquier cuenta a quien vea la consola. |
