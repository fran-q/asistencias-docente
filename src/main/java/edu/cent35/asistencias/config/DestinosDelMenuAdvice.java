...
public class DestinosDelMenuAdvice {
    ...
    public void destinos(Authentication auth, Model model) {
        ...
        model.addAttribute("destinoAcademico",
            seccionesService.destinoDelGrupo(SeccionesService.Grupo.ACADEMICO, auth));
        model.addAttribute("destinoAsistencia",
            seccionesService.destinoDelGrupo(SeccionesService.Grupo.ASISTENCIA, auth));
        model.addAttribute("destinoPersonal",
            seccionesService.destinoDelGrupo(SeccionesService.Grupo.PERSONAL, auth));
    }
}
