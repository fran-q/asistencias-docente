...
(function (window, document) {
    ...
    // El modo inicial NO se decide aca: lo mide un script inline del layout, apenas
    // se cierra el <header> y antes del primer paint. Este archivo carga con defer,
    // o sea despues, y el navegador puede haber pintado ya. Lo que sigue se ocupa de
    // las interacciones y de los cambios de tamano.
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Ancho con el que se decidio la ultima vez. ResizeObserver dispara su callback
        // apenas se lo conecta, con el tamano que el elemento YA tenia, asi que sin esto
        // cada carga volvia a medir sin que nada hubiera cambiado.
        var ultimoAncho = null;

        function evaluateLayout() {
            // Se compara el ancho del navbar y no el de la ventana: es la magnitud de la
            // que depende la decision, y ademas capta lo que innerWidth no ve, como la
            // barra de scroll vertical apareciendo cuando el contenido crece.
            if (ultimoAncho === navbar.clientWidth) {
                return;                     // nada cambio: no hay nada que volver a decidir
            }
            ultimoAncho = navbar.clientWidth;

            // Medir obliga a sacar nav-compact un instante, y eso devuelve el menu a su
            // posicion natural: A LA VISTA. Con las transiciones activas, reponer la clase
            // hace que el drawer se DESLICE hasta esconderse, encima de la pagina ya
            // cargada. Es el mismo parpadeo de siempre, pero disparado por la medicion y no
            // por el primer paint, y por eso sobrevivio al arreglo anterior.
            //
            // Se suprimen mientras dura la medicion y se reponen recien con el estado final
            // ya asentado.
            var yaSuprimidas = body.classList.contains('nav-preload');
            if (!yaSuprimidas) body.classList.add('nav-preload');
            ...
            if (!yaSuprimidas) {
                // Este reflow no sobra: deja asentado el estado final ANTES de devolver
                // las transiciones. Sin el, el navegador toma el cambio recien hecho como
                // algo que hay que animar, que es exactamente lo que se quiere evitar.
                // eslint-disable-next-line no-unused-expressions
                void navbar.offsetWidth;
                body.classList.remove('nav-preload');
            }

            // Ya no se guarda la decision en sessionStorage. Servia para que la
            // pagina siguiente arrancara en el modo correcto, pero fallaba en la
            // primera carga de la sesion, cuando quedaba guardado el modo de una
            // ventana ancha, y en modo privado: en esos casos la pagina se pintaba
            // con el menu entero desplegado y despues saltaba al drawer. Ahora el
            // modo se mide antes del primer paint en el layout, que no tiene esos
            // huecos, y esta funcion solo se ocupa de los cambios de tamano.
        }
